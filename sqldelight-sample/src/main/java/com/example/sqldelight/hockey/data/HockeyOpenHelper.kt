...
object HockeyOpenHelper : SupportSQLiteOpenHelper.Callback(DATABASE_VERSION) {
  ...
  private fun createQueryWrapper(database: SqlDatabase): QueryWrapper {
    return QueryWrapper(
        ...
        playerAdapter = Player.Adapter(
            ...
            shootsAdapter = EnumColumnAdapter.create(Shoots.values()),
            positionAdapter = EnumColumnAdapter.create(Position.values())
        )
    )
  }
  ...
  fun getInstance(context: Context): QueryWrapper {
    if (instance == null) {
      instance = createQueryWrapper(QueryWrapper.wrapContext(
          context = context,
  ...
  override fun onCreate(db: SupportSQLiteDatabase) {
    val sqlDatabase = QueryWrapper.wrapDatabase(db)
  ...
  override fun onUpgrade(
    ...
    val queryWrapper = createQueryWrapper(QueryWrapper.wrapDatabase(db))
    when (oldVersion) {
      1 -> {
        queryWrapper.teamQueries.updateCoachForTeam("Randy Carlyle", "Anaheim Ducks")
      }
    }
  }
